...
// ── COMPATIBILITÀ: carica dati dinamici su window al boot ─────
async function _initDynamicData() {
  const [events, vending] = await Promise.all([
    DB.getEvents(),
    DB.getVending(),
  ]);
  window.EVENTS  = events.length  ? events  : EVENTS_FALLBACK;
  window.VENDING = vending.length ? vending : VENDING_FALLBACK;
}

// Dati di fallback (mostrati mentre Supabase carica)
const EVENTS_FALLBACK = [];
const VENDING_FALLBACK = [];

// Avvia al caricamento
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _initDynamicData);
} else {
  _initDynamicData();
}
Object.assign(window, {
  QUOTES, quoteOfTheDay, SERVICES, PLANS, PT_PACKS, STATUS, DB,
  EVENTS: [], VENDING: [], // inizializzati vuoti, poi riempiti da _initDynamicData
});
>>>>>>> eedc449bd446be50e4e53dd9dcf87dcfb9a81cf6
